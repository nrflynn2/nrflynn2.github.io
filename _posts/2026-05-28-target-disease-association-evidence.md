...
A useful target-disease evidence package usually includes (a) genetic evidence where it exists, as the anchor, (b) transcriptomic and proteomic evidence in disease tissue that is consistent with the genetic signal, (c) tissue-specificity data (see [tissue-specificity evidence] → *Tissue specificity: the safety half of target selection*[^tissue]), and (d) network or pathway context that situates the target in plausible biology. No single layer is enough - the layers have to agree.
